...
Since we are now more familiar with clases and objects, 
we will also introduce the ``StringBuilder`` class to you.
``StringBuilder`` allows you to create strings from collection types and Arrays.
It is common to see these objects paired with a loop.
...
Up until this point, if you wanted to turn a collection of strings objects 
into a *single* string object, you could concatenate them using a loop.

For example, if we have an array of strings containing each word in a sentence, 
we may want to concatenate each value in the array to reform our sentence.
...
`Here is another example <https://replit.com/@launchcode/StringBuilderExample-CSharp#main.cs>`_, if you would like to practice more with this class.
